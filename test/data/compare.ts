...
test("Compare<string>", (t) => {
  ...
  t.deepEqual(sortBy(stringCompare)(["cc", "aa", "bb", "a_"]), [
    "a_",
    "aa",
    "bb",
    "cc",
...
test("Constravarint", (t) => {
  ...
  t.deepEqual(sortBy(lengthCompare)(["cc", "aaa", "", "b"]), [
    "",
    "b",
    "cc",
    "aaa",
...
test("Semigroup", (t) => {
  ...
  t.deepEqual(sortBy(lengthThenAlphaCompare)(["cc", "aa", "", "b"]), [
    "",
    "b",
    "aa",
    "cc",
...
test("Monoid", (t) => {
  ...
  const lengthThenAlphaCompare = [lengthCompare, stringCompare].reduce(
    concat,
    empty
  );
...
test("Divisible", (t) => {
  t.is(conquer("", "a"), 0);
  t.is(conquer("cc", "a"), 0);
  t.is(conquer("aa", "aa"), 0);

  type Person = { name: string; age: number };
  const Michel = { name: "Michel", age: 51 };
  const Patrick = { name: "Patrick", age: 42 };

  const personCompare = divide(
    (person: Person) => [person.name, person.age],
    stringCompare,
    numberCompare
  );

  t.is(personCompare(Michel, Patrick), -1);
  t.is(personCompare(Michel, { ...Michel, age: 50 }), 1);
  t.is(personCompare(Michel, Michel), 0);
});
...
test("Decidable", (t) => {
  ...
  const Titanic: Product = {
    type: "movie",
    title: "Titanic",
    duration: 195,
  };
  const DieHard: Product = {
    type: "movie",
    title: "DieHard",
    duration: 132,
  };
  ...
  const TheFellowshipOftheRing: Product = {
    type: "book",
    title: "The Fellowship of the Ring",
    pages: 423,
  };

  const productCompare = choose(
    (product: Product) => {
      switch (product.type) {
        case "book":
          return ["left", product.pages];
        case "movie":
          return ["right", product.duration];
      }
    },
    numberCompare,
    numberCompare
  );
  t.is(productCompare(Titanic, HarryPotter), 1);
  t.is(productCompare(Titanic, DieHard), 1);
  t.is(productCompare(HarryPotter, TheFellowshipOftheRing), -1);

  t.deepEqual(
    sortBy(productCompare)([
      Titanic,
      DieHard,
      HarryPotter,
      TheFellowshipOftheRing,
    ]),
    [HarryPotter, TheFellowshipOftheRing, DieHard, Titanic]
  );
});
...
test("Progression", async (t) => {
  ...
  const aggregations: Array<Aggregation> = map<Progression, Aggregation>(
    (progression) => ({
  ...
  t.deepEqual(
    sortBy<Aggregation>(
      concat(
        contramap(numberCompare, (aggre) => aggre.level),
  ...
  t.deepEqual(
    sortBy<Progression>(
      concat(
        contramap(numberCompare, (progression) => LEVEL[progression.level]),
        inverse(
          contramap(numberCompare, (progression) =>
            new Date(progression.createdAt).getTime()
          )
        )
      )
    )(progressions),
